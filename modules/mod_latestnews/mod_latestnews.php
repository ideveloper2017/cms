...
function mod_latestnews($id){
    ...
    $news = $db->query("SELECT c.*,ct.cname as category,DATE_FORMAT(c.created_at,'%H:%i %d.%M %Y ') as pubdate FROM content c inner join categories_bind cb on cb.item_id=c.id
                                                                       inner join categories ct on ct.id=cb.category_id
                          where c.active=1 {$where} and c.created_at<='{$today}' and (c.is_end=0 or (c.is_end=1 and c.end_at>='{$today}'))
                          and c.lang='{$langID}'
...
//        $items[$inc]['image'] = file_exists(PATH.'/images/content/' . $conrows->images)? $conrows->images:'';
        if ($cfg['first_img']){
            ...
            $images=!empty($conrows->images)?'/images/content/'. $conrows->images:'';
            $images_medium=!empty($conrows->images)?'/images/content/medium/'. $conrows->images:'';
            $images_small=!empty($con['images'])?'/images/content/small/'. $conrows->images:'';
            $items[$inc]['image'] = (file_exists(PATH.$images) ? $images : '/modules/mod_news_block3/nopic.jpg');
            $items[$inc]['image_medium'] = (file_exists(PATH.$images_medium) ? $images_medium : '/modules/mod_news_block3/nopic.jpg');
        ...
        $items[$inc]['pubdate'] = $core->cmsRusDate($conrows->pubdate);
...
//        $items[$inc]['comments'] =$conrows->comments;
    ...
    include(PATH . '/theme/' . Registry::get("Config")->template . '/modules/mod_latestnews.php');
}
?>