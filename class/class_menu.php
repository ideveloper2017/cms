...
//    private $menutree = array();
    ...
    public function getMenu($array, $parent_id = 1, $menuid = 'menu-main-menu', $class = ' class="main-menu"')
    {
        if (is_array($array) && count($array) > 0) {
            ...
            $attr = ($parent_id==1) ? ' class="' . $class .'"' : '';
            $attr2 = ($parent_id!=1) ? 'dropdown':'  ' ;
...
//                   echo  ' class="dropdown" ';
//                }
//                $attr2 = ($row['id']==$row['parent_id']) ? ' ok      ':' not ok ' ;
//                $attr2 = ($parent_id==$row['id']) ? ' class="dropdown" ':'  ' ;
//                echo $attr2;
                if ($row['parent_id'] == $parent_id) {
                    ...
                    if ($submenu === false) {
                        $submenu = true;
                        $selarrov= "";
//                        $attr2 = ' class="dropdown" ';

                        print "<ul" . $attr . ">\n";
                    }
                    $active = ($row['pslug'] == $this->slug) ? 'current' : '';
                    $mactive = ($row['pslug']) ? "active" : "normal";
                    $cssclass = $row['css_class']?$row['css_class']:'';
                    $name = ($parent_id == 1)  ? '<strong>' . $row['title'] . '</strong>' : $row['title'];
                    $homeactive = (preg_match('/index.php', $_SERVER['PHP_SELF'])) ? 'current' : "";
...
//                            $murl =  Url::Photo($row['slug']).'.html';
                            $murl = $row['linkid'];
                            $murl2 = $row['home'] ? '/' : $murl;
                            $link = '<a href="/' . $murl . '" >' . $icon . $caption . '</a>';
                            break;

                        case 'content':
                            $murl = $row['id'];
                            $murl2 = $row['link'] ? '/' : $murl;
                            $link = '<a href="'. $row['link']  . '">' . $icon . $caption . '</a>';
                            break;

                        case 'module':
                            $murl = $row['slug'] . '/';
                            $murl2 = $row['home'] ? '/' : $murl . ".html";
                            $link = '<a href="' . $murl2 . '" >' . $icon . $caption . '</a>';
                            break;

                        case 'category':
                            $url = '/'.$row['id'].'/'.$row['link'];
                            if ($row['home']) {
                                $link = '<a href="/" >' . $icon . $caption . '</a>';
                            } else {
                                $link = '<a href="' . $url . '" >' . $icon . $caption . '</a>';
                            }
                            break;
                        ...
                        case 'link':
                            ...
                            if ($row['home']) {
                                ...
                                $link = '<a href="' .$wlink . '" >' . $icon . $caption . '</a>';
                            }else{
                                $link = '<a href="'.$wlink.'"  >' . $icon . $caption . ' </a>';
                            }
                            break;
                    ...
                    print '<li class="' . $attr2.$active.$homeactive. '" >';
                    print $link;
