...
//        = self::$db->fetch($rows)
    ...
    public function getArticleList($cat_id){
        self::$db->query("SET SQL_BIG_SELECTS=1");
        $where='';
...
//        print_r($categories);
    ...
    public function getTagLine($target, $item_id, $links = true, $selected = '')
    {
        ...
        if ($tags) {
            ...
            while ($tag = self::$db->fetch($rs)) {
                if ($links){
                    if ($selected==$tag->tag){
                        $html .= '<span class="blank"><a  href="/search/tag/'.urlencode($tag->tag_name).'">'.$tag->tag_name.'</a></span>';
                    }else{
                        $html .= '<span class="blank"><a href="/search/tag/'.urlencode($tag->tag_name).'">'.$tag->tag_name.'</a></span>';
                    }
                }else{
    ...
    public function getTagItemLink($target, $item_id){
        ...
        $link = '';
        $cfg = $inCore->getComponentConfig("content");
        $content = array();
        if (!isset($cfg['first_img'])) { $cfg['first_img'] = 1; }
        switch ($target){
            case 'article': $today = date("Y-m-d H:i:s");
                $sql = "SELECT i.title as title, c.cname as cat, i.seo as seolink, c.slug as cat_seolink,i.images,i.introtext,i.fulltext,i.created_at as pubdate,i.hits
								FROM content i
                ...
                if ($inDB->numrows($rs)){
                    $item = $inDB->first($rs);
                    $menu_id = $inDB->getFieldById('id', 'menus', "alias='{$item->cat_seolink}'");
                    if ($cfg['first_img']){
                        ...
                        if ($matches[1]){
                            ...
                            if (preg_match('/http|https/', $test_out)){
                                ...
                                if ($law_img) {
                                    $content['first_image'] = $test_out;
                                }
                            }
                            else if (file_exists(PATH.$matches[1])) {
                                $content['first_image'] = $matches[1];
                            }
                        } else {
                            ...
                            $content['image'] = (file_exists(PATH.$images) ? $images : '/modules/mod_news_block3/nopic.jpg');
                            $content['image_medium'] = (file_exists(PATH.$images_medium) ? $images_medium : '/modules/mod_news_block3/nopic.jpg');
                            $content['image_small'] = (file_exists(PATH.$images_small) ? $images_medium : '/modules/mod_news_block3/nopic.jpg');
                        }
                    } else {
                        $images=!empty($item->images)?'/images/content/'. $item->images:'';
                        $images_medium=!empty($item->images)?'/images/content/medium/'. $item->images:'';
                        ...
                        $content['image'] = (file_exists(PATH.$images) ? $images : '/modules/mod_news_block3/nopic.jpg');
                        $content['image_medium'] = (file_exists(PATH.$images_medium) ? $images_medium : '/modules/mod_news_block3/nopic.jpg');
                        $content['image_small'] = (file_exists(PATH.$images_small) ? $images_small : '/modules/mod_news_block3/nopic.jpg');
                    }



                    $link.='<div class="col-md-4 col-sm-4 col-xs-12">
                <div class="item-related-article">
                    <div class="related-article-thumb">';
                    if ($content['image']){
                        $link.='<a href="/content/'.$item->seolink.'.html"><img src="'.$content['image'].'" alt="UniqMag" style="width:100%;height:204px;"></a>';
                    } else {
                        $link.='<a href="/content/'.$item->seolink.'.html"><img src="'.$content['first_image'].'" alt="UniqMag" style="width:333px;height:204px;"></a>';
                    }
                        $link.='<ul class="featurred-cat-newday">
                            
                            <li><span class="lnr lnr-clock"></span> '. $item->pubdate.'</li>
                            <li><span class="lnr lnr-eye"></span> '. $item->hits.'</li>
                        </ul>
                    </div>
                    <div class="related-article-info">
                        <h3><a href="'.Content::getArticleURL($menu_id, $item->seolink).'">'.$item->title.'</a></h3>
                    </div>
                </div>
            </div>';

//                    $link.='<div class="cs-col cs-col-4-of-12"><div class="cs-post-block-layout-3"><div class="cs-post-item">
//                                        <div class="cs-post-thumb">
//                                            <div class="cs-post-category-border cs-clearfix">
//                                                <a href="/'.$item->cat_seolink.'-'.$menu_id.'">'.$item->cat.'</a>
//                                            </div>';
//
//                    if ($content['image']){
//                        $link.='<a href="/content/'.$item->seolink.'.html"><img src="'.$content['image'].'" alt="UniqMag" style="width:100%;height:204px;"></a>';
...
//                                            <h3><a href="/content/'.$menu_id.'/'.$item->seolink.'.html">'.$item->title.'</a></h3>
